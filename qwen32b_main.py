# -*- coding: utf-8 -*-
"""
@File    : qwen32b_main.py
@Author  : qy
@Date    : 2025/9/10 09:25
"""
...
from policy_utils_localdata import get_policy_info
...
class NewCheckRequest(BaseModel):
    ...
    user_input_text: Optional[str] = Field(None, description="用户输入的文本",max_length=200)
    check_mode: int = 1
    ...
    def remove_empty_values(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                # 先清理子字典，再判断是否为空
                cleaned_sub = BaseUtils.remove_empty_values(value)
                if cleaned_sub:  # 子字典清理后不为空才保留
                    clean_data[key] = cleaned_sub
                continue
...
async def check_policy(req: NewCheckRequest):
    if req.check_mode == 1:
        ...
        policy_elements_prompt = build_policy_elements_prompt(req.part_id)
        ...
        async def event_stream() -> AsyncGenerator[Dict[str, Any], None]:
            ...
            async def run_standardization() -> Dict[str, Any]:
                ...
                if not req.user_input_text or not req.user_input_text.strip():
                    ...
                    logger.info(f"====当前公司信息（用户没有输出）====：{json.dumps(cleaned, ensure_ascii=False, indent=2)}")
                ...
                logger.info(f"====更新后企业信息====：{json.dumps(cleaned, ensure_ascii=False, indent=2)}")
                return cleaned
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run(app="qwen32b_main:app", workers=2, host="0.0.0.0", port=8000, reload=False)
